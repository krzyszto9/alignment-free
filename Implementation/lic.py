...
def angle_cos_distance(seqs_number, occurences_list):
    ...
    for i in range(0,seqs_number):
        for j in range(i,seqs_number):
             matrix[i][j] = matrix[j][i] = 1 - angle_cos(i,j,occurences_list)
    return matrix
...
def evolutionary_distance(seqs_number, occurences_list):
    ...
    for i in range(0,seqs_number):
        for j in range(i,seqs_number):
              matrix[i][j] = matrix[j][i] =-np.log((1+angle_cos(i,j,occurences_list))/2)
    return matrix
...
def pearson(seqs_number, frequencies_list):
    ...
    for i in range(0,seqs_number):
        for j in range(i,seqs_number):
             l= np.count_nonzero(frequencies_list[i,:] + frequencies_list[j,:])
             matrix[i][j] = matrix[j][i] =(l * np.sum(frequencies_list[i,:]*frequencies_list[j,:]) - (np.sum(frequencies_list[i,:]) * np.sum(frequencies_list[j,:])))/ np.sqrt((l*np.sum(frequencies_list[i,:]**2)-np.sum(frequencies_list[i,:])**2)*(l*np.sum(frequencies_list[j,:]**2)-np.sum(frequencies_list[j,:])**2))
    return matrix
    ...
    for i in range(0,seqs_number):
        for j in range(0,seqs_number):
            if i ==1 and j==0:
                ...
                frequencies_list[frequencies_list == 0] = x
                log2 = np.where(np.logical_and(frequencies_list[i,:]!=0,frequencies_list[j,:]!=0),np.log(frequencies_list[i,:]/frequencies_list[j,:]),0)
                print log2
                print frequencies_list[i,:]
                matrix[i][j] = np.sum(log2 * frequencies_list[i,:]) 
    return matrix'''

arguments = parse()
seqs_number = teiresias_run(arguments.lenght, arguments.input.name)
occurences_list = teiresias_patterns(seqs_number)    #można zastanowic sie nad poprawa, tak samo frequencies list (lub z pliku frequencies_list)
np.set_printoptions(suppress=True)
#Dystans euklidesowy
print "Euclidean distance matrix\n", euclidean_distance(seqs_number, occurences_list)
#Cosinus kata pomiedzy sekwencjami
print "Angle metrics\n", angle_cos_distance(seqs_number, occurences_list)          #minusiwe zera
#dystans ewolucyjny
print "Evolutionary distance\n", evolutionary_distance(seqs_number, occurences_list)   #minusiwe zera
frequencies_list = calculate_frequencies(seqs_number, occurences_list)			
#wspolczynnik korelacji liniowej Pearsona
print "Pearson product-moment correlation coefficient\n", pearson(seqs_number, frequencies_list) #minus dla tych samych sek dla a.txt
